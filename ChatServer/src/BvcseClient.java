    ...
    extends Thread
{
	...
	synchronized (istream) {
		...
		if (istream.readShort() == BvcseCommand.SETPROTO)
		    receiveProtocol();
		else
		    {
			...
			new BvcseTextDialog(mainWindow, "Warning",
	...
	synchronized (istream) {
	    ...
	    if (protocolVersion >= 2.0)
		{
		    ...
		    case BvcseCommand.NOOP:
			{
			    // Do nothing
			    break;
			}

		    case BvcseCommand.PING:
			{
		    ...
		    case BvcseCommand.RECT:
			{
			    // Someone drew a rectangle on the canvas
			    receivePoly(BvcseCommand.RECT);
			    break;
			}

		    case BvcseCommand.OVAL:
			{
			    // Someone drew an oval on the canvas
			    receivePoly(BvcseCommand.OVAL);
			    break;
			}

		    case BvcseCommand.DRAWTEXT:
			{
		    ...
		    default:
			{
			    ...
			    // tried to connect to a Bvcse Chat v1.x server
			    // that doesn't speak our language?
			    String stringFoo = new String(foo);
			    if (stringFoo.startsWith("lcome to Bvcse"))
				{
				    notifyObsoleteV1();
				    shutdown(false);
	...
	// a Bvcse Chat v1.x server, and can't accept a connection from
	// this client.
	new BvcseInfoDialog(parentWindow, "Obsolete server", true,
			      "Cannot communicate with a version 1.x "
			      + "server.  Please ask the server operator to "
			      + "upgrade.");
    }

    protected BvcseUser findUser(int userId)
    {
	// Find a user in the list

	BvcseUser tmpUser = null;
	BvcseUser returnUser = null;
	...
	for (int count = 0; count < userList.size(); count ++)
	    {
		tmpUser = (BvcseUser) userList.elementAt(count);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.SETPROTO);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.PING);
	    }
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.USERINFO);
	...
	if (tmpName.equals(parentWindow.name))
		...
		newUser = new BvcseUser(tmpId, tmpName, "", tmpAdditional);
	...
	new BvcseInfoDialog(parentWindow, "Server message", true,
			      message);
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.DISCONNECT);
	...
	if ((tmpId == parentWindow.id) || (tmpId == 0))
		...
		new BvcseInfoDialog(parentWindow, "Disconnected", true,
				      disconnectMess);
		...
		// If there's a BvcseRoomControl dialog, tell it to update
		// itself
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.ROOMLIST);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.INVITE);
		ostream.writeInt(parentWindow.id);
	...
	new BvcseInfoDialog(parentWindow, "Invitation", false,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.ENTERROOM);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
	...
	if (userId == parentWindow.id)
		...
		for (int count1 = 0; count1 < userList.size(); count1 ++)
		    {
			tmpUser = (BvcseUser) userList.elementAt(count1);
		...
		if (parentWindow.currentRoom.creatorName
			...
			new BvcseRoomControl(parentWindow);
	...
	// If there's a BvcseRoomControl dialog, get a room list update
	if (parentWindow.roomControlDialog != null)
	    requestRoomList();
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.BOOTUSER);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
		ostream.writeInt(userId);
	    }
	...
	// they're a goner.
	new BvcseInfoDialog(parentWindow, "Booted", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.BANUSER);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
		ostream.writeInt(userId);
	    }
	...
	// Make a dialog box with a message so that the user notices
	// they're a goner.
	new BvcseInfoDialog(parentWindow, "Banned", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.ALLOWUSER);
		ostream.writeInt(parentWindow.id);
		ostream.writeUTF(roomName);
		ostream.writeInt(userId);
	    }
	...
	new BvcseInfoDialog(parentWindow, "Allowed", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.ACTIVITY);
		ostream.writeInt(parentWindow.id);
	...
	String tmpString = "";
	if (activity == BvcseCommand.ACTIVITY_DRAWING)
	    tmpString = "drawing: " + fromUser.name;
	else if (activity == BvcseCommand.ACTIVITY_TYPING)
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.CHATTEXT);
		ostream.writeInt(parentWindow.id);
		if (parentWindow.sendToAll.getState())
		    // Public
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.LINE);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.DRAWTEXT);
		ostream.writeInt(parentWindow.id);
		ostream.writeShort(colour);
		ostream.writeShort(x);
		ostream.writeShort(y);
	...
	try {
		...
		{
		    new BvcseInfoDialog(parentWindow, "Error", true,
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.DRAWPICTURE);
		ostream.writeInt(parentWindow.id);
		ostream.writeShort(x);
		ostream.writeShort(y);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.CLEARCANV);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.PAGEUSER);
		ostream.writeInt(parentWindow.id);
	...
	if (parentWindow.menuPlaySound.getState())
		...
		float javaVersion = Float
		    .parseFloat(System.getProperty("java.version")
				.substring(0, 3));
		if (javaVersion < 1.2)
		    sendError(fromUser.id, BvcseCommand.ERROR_NOSOUND);
			...
			try {
			    URL soundURL =
				new URL(parentWindow.BvcseURL.getProtocol(),
					parentWindow.BvcseURL.getHost(),
					parentWindow.BvcseURL.getFile() +
					"BvcsePage.au");
			    ...
			    sendError(fromUser.id,
				      BvcseCommand.ERROR_NOSOUND);
			}
		    }
	    }
	else
	    // The user is not accepting pages.  Send a message.
	    sendError(fromUser.id, BvcseCommand.ERROR_NOPAGE);
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.ERROR);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.INSTANTMESS);
		ostream.writeInt(parentWindow.id);
	...
	new BvcseTextDialog(parentWindow, "Instant message from "
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.LEAVEMESS);
		ostream.writeInt(parentWindow.id);
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.READMESS);
		ostream.writeInt(parentWindow.id);
	    }
    }
	...
	if (numberMessages == 0)
	    {
		new BvcseInfoDialog(parentWindow, "None", true,
				      "No unread messages."); 
		return;
	    }
	...
	for (int count = 0; count < numberMessages; count ++)
		...
		// Place it on the screen as a dialog box
		new BvcseTextDialog(parentWindow, "Message from " + from,
			      message, 40, 10,
			      TextArea.SCROLLBARS_VERTICAL_ONLY, true);
	    }

	// Tell the user that that's all
	new BvcseInfoDialog(parentWindow, "Done", true,
	...
	switch(errorCode)
	    {
	    case BvcseCommand.ERROR_NOPAGE:
		{
	    ...
	    case BvcseCommand.ERROR_NOSOUND:
		{
