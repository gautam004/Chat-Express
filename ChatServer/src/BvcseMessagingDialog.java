...
public class BvcseMessagingDialog
    extends Dialog
    implements ActionListener, ItemListener, KeyListener, WindowListener
{
    ...
    public BvcseMessagingDialog(BvcseWindow parent)
    {
	...
	if ((instantUser == null) && saveUser.equals(""))
	    {
		new BvcseInfoDialog(parentWindow, "Need recipient", true,
	...
	if (parentWindow.connected != true)
	    {
		new BvcseInfoDialog(parentWindow, "Not connected", true,
	...
	if (!saveUser.equals(""))
		...
		for (int count = 0; count < usersVector.size(); count ++)
		    {
			BvcseUser user =
			    (BvcseUser) usersVector.elementAt(count);
