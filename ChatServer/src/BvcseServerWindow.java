...
public class BvcseServerWindow
    extends Frame
    implements ActionListener, ItemListener, WindowListener
{
    ...
    protected BvcsePictureCanvas canvas;
    protected BvcseServer myParent;
    protected BvcseWindow consoleWindow;


    public BvcseServerWindow(BvcseServer parent, String Name)
    {
	super(Name);
	myParent = parent;
	...
	canvas = new BvcsePictureCanvas(this);
	myConstraints.gridwidth = 2; myConstraints.gridheight = 1; 
	...
	try {
	    URL url = new URL ("file", "localhost", "BvcsePic.jpg");
	...
	try {
	    URL iconUrl = new URL("file", "localhost", "BvcseIcon.jpg");
	...
	if (E.getSource() == userAdmin)
	    {
		BvcseUserToolDialog userTool =
		    new BvcseUserToolDialog(this);
		return;
	    }
	...
	if (E.getSource() == console)
	    {
		BvcseInfoDialog tmp =
		    new BvcseInfoDialog(this, "Loading", false,
			  "Starting the client, one moment please...");
		consoleWindow =
		    new BvcseWindow("Administrator", "", "localhost",
	...
	if (E.getSource() == shutdown)
		...
		if (myParent.currentConnections > 0)
		    new BvcseServerShutdownDialog(this, myParent);
		else
		    myParent.shutdown();
		return;
	    }
	...
	if (E.getSource() == logChat)
		...
		for (int count = 0; count < myParent.chatRooms.size();
		     count ++)
		    {
			BvcseChatRoom tmp = (BvcseChatRoom)
...
class BvcseServerShutdownDialog
    extends Dialog
    implements ActionListener, KeyListener, WindowListener
{
    protected BvcseServerWindow myParent;
    protected BvcseServer theServer;
    ...
    public BvcseServerShutdownDialog(BvcseServerWindow serverwindow,
				       BvcseServer server)
    {
...
class BvcsePictureCanvas
    extends Canvas
{

    private BvcseServerWindow main;
    private Image image;
    
    public BvcsePictureCanvas(BvcseServerWindow mainwindow)
    {
	super();
	main = mainwindow;
	setBackground(Color.lightGray);
