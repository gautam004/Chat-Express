	       ...
	       WindowListener, ClipboardOwner
{
    ...
    protected BvcseRoomInfo currentRoom;
    protected BvcseRoomInfo[] roomInfoArray;
    ...
    protected BvcsePasswordEncryptor passwordEncryptor = null;

    protected BvcseRoomsDialog roomsDialog;
    protected BvcseRoomControl roomControlDialog;

    // socket stuff

    public BvcseClient theClient;
    protected boolean connected;

    // If we're operating as an applet
    protected BvcseApplet thisApplet = null;
    ...
    public BvcseWindow(String userName, String userPassword,
	...
	menuAbout = new MenuItem("About Bvcse Chat");
	...
	try {
	    URL imageUrl =
		new URL(BvcseURL.getProtocol(), BvcseURL.getHost(),
		BvcseURL.getFile() + "BvcsePic.jpg");
	...
	// applicable
	passwordEncryptor = new BvcsePasswordEncryptor();
	...
	try {
	    URL iconUrl =
		new URL(BvcseURL.getProtocol(), BvcseURL.getHost(),
			BvcseURL.getFile() + "BvcseIcon.jpg");
	...
	if (adminConsole)
	    roomOwner(true);
	setTitle("Bvcse Chat v" + Bvcse.VERSION + " - online at " + host);
	return;
    }
	...
	currentRoom = new BvcseRoomInfo();
	roomOwner(false);
	setTitle("Chat Express" + Bvcse.VERSION + " - offline");
	return;
    }
	...
	if (!haveRequiredInformation())
		...
		new BvcseSettings(this);
		
		// Do we have it NOW?
		if (!haveRequiredInformation())
		    {
			new BvcseInfoDialog(this, "Connection canceled",
					      true,
	...
	try {
	    theClient = new BvcseClient(host, name, portNumber, this);
	}
	catch (UnknownHostException a) {
	    new BvcseInfoDialog(this, "Couldn't connect", true,
		...
		errorString += "\nIf you think this is a bug, please send a "
		    + "bug report to info@Bvcseitc.com\n";

		new BvcseTextDialog(this, "Couldn't connect",
				      errorString, 60, 15,
				      TextArea.SCROLLBARS_VERTICAL_ONLY,
				      true);
		return;
    ...
    protected void setApplet(BvcseApplet ap)
    {
	...
	for (int count1 = 0; count1 < selectedUsers.length; count1 ++)
		...
		for (int count2 = 0; count2 < theClient.userList.size();
		    ...
		    {
			BvcseUser tmp =
			    (BvcseUser) theClient.userList.elementAt(count2);
	...
	try {
	    ...
	    new BvcseInfoDialog(this, "Failed", true,
	...
	if (!pictureFile.canRead())
		...
		new BvcseInfoDialog(this, "Permission denied", true,
	...
	if (E.getSource() == menuChatRooms)
		...
		if (roomsDialog != null)
		    roomsDialog.dispose();
		roomsDialog = new BvcseRoomsDialog(this);
		return;
	    }
	...
	if (E.getSource() == menuChatRoomControl)
		...
		if (roomControlDialog != null)
		    roomControlDialog.dispose();
		roomControlDialog = new BvcseRoomControl(this);
		return;
	    }
	...
	if (E.getSource() == menuManual)
	    {
		try {
		    URL manualURL = new URL(BvcseURL.getProtocol(),
				    BvcseURL.getHost(),
				    BvcseURL.getFile() + "MANUAL.TXT");
		    ...
		    new BvcseTextDialog(this, "Bvcse Online Manual", 
				  input, 65, 25,
				  TextArea.SCROLLBARS_VERTICAL_ONLY, false);
		    return;
		}
		catch (IOException G) {
		    new BvcseInfoDialog(this, "Data not available", 
			  true,
	...
	if (E.getSource() == menuAbout)
	    {
String abouttext = new String("Bvcse v" + Bvcse.VERSION + "\nCopyright Bvcse IT CONSULTING (P) LTD.\n\n");
		
		new BvcseTextDialog(this, "About Bvcse Chat", 
				      abouttext, 60, 22,
	...
	if (E.getSource() == typed)
		...
		if (E.getKeyCode() == E.VK_ENTER) 
			...
			if (connected == true)
				...
				theClient.sendActivity(BvcseCommand
						       .ACTIVITY_TYPING);
			    }
			    catch (IOException e) {
				theClient.lostConnection();
