    ...
    implements ActionListener, ItemListener, KeyListener, WindowListener
{
	...
	if (index >= 0)
		...
		if (parentWindow.currentRoom == selectedRoom)
		    {
			new BvcseInfoDialog(parentWindow,
		...
		if (selectedRoom.priv && !selectedRoom.invited)
		    {
			BvcsePasswordDialog passDialog = 
			    new BvcsePasswordDialog(parentWindow,
	...
	if (index >= 0)
		...
		new BvcseTextDialog(parentWindow, "Chat room information",
				      infoString, 30, 10,
	...
	if (E.getSource() == createRoom)
	    {
		new BvcseCreateRoom(parentWindow);
		dispose();
		return;
	    }
