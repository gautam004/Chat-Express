    ...
    extends Thread
{
    ...
    private BvcseServer server;
    
    public BvcseServerShutdown(BvcseServer s)
    {
	server = s;
    }
    ...
    extends Thread
{
    ...
    protected URL myURL;
    protected int port;
    protected BvcseServerWindow myWindow;
    ...
    public BvcseChatRoom mainRoom;
    public Vector chatRooms;

    private FileOutputStream log;
    protected BvcsePasswordEncryptor passwordEncryptor;

    protected BvcseClientSocket administratorClient;
    ...
    public BvcseServer(int askport, boolean passwords, boolean newusers,
	...
	if (graphics)
	    {
		myWindow =
		    new BvcseServerWindow(this, "Chat Express server");
	...
	mainRoom = new BvcseChatRoom("Main", "Administrator", false, null);
	try {
	...
	if (javaVersion >= 1.3)
	    Runtime.getRuntime().addShutdownHook(new BvcseServerShutdown(this));
    ...
    protected BvcseChatRoom findChatRoom(String roomName)
    {
	// This will return the chat room object with the corresponding
	// name
	
	BvcseChatRoom tempRoom = null;
	BvcseChatRoom returnRoom = null;
    ...
    public boolean isUserAllowed(BvcseChatRoom room,
			       BvcseClientSocket client, String password)
    {
    ...
    public synchronized void disconnect(BvcseClientSocket who,
					boolean notify)
    {
	int count;
	BvcseChatRoom chatRoom;
	...
	for (count = 0; count < currentConnections; count ++)
	    {
		BvcseClientSocket other = (BvcseClientSocket)
	...
	for(count = (currentConnections - 1); count >= 0; count --)
	    {
		BvcseClientSocket temp =
		    (BvcseClientSocket) connections.elementAt(count);
	...
	while (!stop) 
		...
		BvcseClientSocket cs = 
		    new BvcseClientSocket(this, mySocket, myThreadGroup);
	    }
    }
	...
	new BvcseUserTool().createUser(userName, encryptedPassword);
    }
	...
	try {
	    ...
	    while(true)
		{
		    ...
		    messages.addElement(new BvcseMessage(tempFor, tempFrom,
							   tempMessage));
		}
	...
	try {
		...
		{
		    BvcseMessage tempMessage =
			(BvcseMessage) messages.elementAt(count);
	...
	if (currentConnections > 0)
		...
		for (int count = 0; count < currentConnections; count ++)
		    {
			BvcseClientSocket who = (BvcseClientSocket)
			    connections.elementAt(count);
			try {
	...
	if (!graphics)
	    {
		System.out.println("");
		System.out.println("Bvcse server shutdown complete");
	    }

	// This function can be called by the BvcseServerShutdown thread
    ...
    {
	System.out.println("\nBvcse Chat server usage:");
	System.out.println("java BvcseServer [" +
	...
	for (int count = 0; count < args.length; count ++)
		...
		if (args[count].equals(portnumberParam))
			...
			System.out.println("\nBvcseServer: unknown "
					   + "argument " + args[count]);
			System.out.println("Type 'java BvcseServer -help' "
					   + "for usage information");
			System.exit(1);
		    }
	    }
	...
	// Start the server
	server = new BvcseServer(usePort, reqPass, allowNew, useGraphics,
				   logChats);
