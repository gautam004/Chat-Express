...
class BvcseLine
{
    ...
    public BvcseLine(Color mycolor, int mystartx, int mystarty,
...
class BvcseRectangle
{
    ...
    public BvcseRectangle(Color mycolor, int myx, int myy, int mywidth,
...
class BvcseOval
{
    ...
    public BvcseOval(Color mycolor, int myx, int myy, int mywidth,
		       int myheight, boolean myfill, int mythickness)
    {
	color = mycolor;
	x = myx;
...
class BvcseText
{
    ...
    public BvcseText(Color mycolor, int myx, int myy, int myfontnumber,
...
class BvcsePicture
{
    ...
    public BvcsePicture(int myX, int myY, int myWidth, int myHeight,
    ...
    extends Canvas
{
    ...
    public String text;
    public BvcseWindow parentWindow;
    
    public BvcseSampleCanvas(BvcseWindow mainWindow)
    {
	super();
	...
	repaint();
	setVisible(true);
	text = new String("Bvcse and on");
    }

    public void paint(Graphics g)
    {
	g.setColor(Color.black);
	g.setFont(new Font(BvcseCanvas
...
class BvcseFontSelect
    extends Dialog
    implements ActionListener, ItemListener, KeyListener, WindowListener
{
    private GridBagLayout myLayout;
    private GridBagConstraints myConstraints;
    private BvcseWindow parentWindow;
    ...
    private BvcseSampleCanvas sample;
    ...
    public BvcseFontSelect(Frame parent)
    {
	super(parent, "Choose font", true);
	parentWindow = (BvcseWindow) parent;
	...
	sample = new BvcseSampleCanvas(parentWindow);
	myConstraints.gridx = 0; myConstraints.gridy = 1;
	myConstraints.gridheight = 1; myConstraints.gridwidth = 1;
	...
	parentWindow.canvas.draftText = 
	    new BvcseText(parentWindow.canvas.drawColor,
    ...
    implements MouseListener, MouseMotionListener
{
    ...
    // For the draft stuff
    private BvcseLine draftLine;
    private BvcseRectangle draftRect;
    private BvcseOval draftOval;
    protected BvcseText draftText;
    protected BvcsePicture draftPicture;
    ...
    public BvcseCanvas(BvcseWindow parent)
    {
	super();
	parentWindow = parent;
	...
	try {
	    ...
	    new BvcseInfoDialog(parentWindow, "Failed", true, e.toString());
	    return;
	}
	...
	draftPicture =
	    new BvcsePicture(10, 10, theImage.getWidth(this),
	...
	g1.setFont(new Font (BvcseCanvas.fontArray[fontnumber],
				    attribs, size));
	g2.setFont(new Font (BvcseCanvas.fontArray[fontnumber],
				    attribs, size));
	...
	if (drawType == this.LINE)
	    {
		draftLine = new BvcseLine(drawColor, oldx, oldy, oldx,
					    oldy, drawThickness);
	    ...
	    {
		draftOval = new BvcseOval(drawColor, oldx, oldy,
	    ...
	    {
		draftRect = new BvcseRectangle(drawColor, oldx, oldy,
						 1, 1, false, drawThickness);
	...
	if (parentWindow.connected)
		...
		parentWindow.theClient
		    .sendActivity(BvcseCommand.ACTIVITY_DRAWING);
	    }
	    catch (IOException e) {
	...
	if (floatingPicture)
		...
		if (floatingText)
		    ...
		    {
			new BvcseFontSelect(parentWindow);
