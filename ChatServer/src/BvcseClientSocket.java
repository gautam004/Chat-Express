...
class BvcseCanvasCommand
{
    ...
    BvcseCanvasCommand(short mycommandtype, int mywhodrew, short mycolour,
...
class BvcseClientTickler
    extends Thread
{
    // This class just runs as a separate thread, pinging the client
    // every second to make sure we haven't lost the connection
     
    BvcseClientSocket client;
 
    public BvcseClientTickler(BvcseClientSocket c)
    {
	super("Bvcse Chat client connection tickler");
    ...
    extends Thread
{
    ...
    public BvcseClientSocket(BvcseServer parent, Socket s, 
			       ThreadGroup threadgroup)
    {
	super("Bvcse Chat client connection");

	server = parent;
	mySocket = s;

	// Get a 'user' object
	user = new BvcseUser(server);

	// Make an initial, dummy chat room
	chatRoom = new BvcseChatRoom("", "", false, "");
	...
	try {
		...
		if (command != BvcseCommand.SETPROTO)
		    {
	...
	synchronized (istream) {
	    ...
	    if (protocolVersion >= 2.0)
		{
		    ...
		    // version 2.0.  See the file BvcseCommand.java for
		    ...
		    default:
			{
			    ...
			    // possibly a Bvcse Chat 1.x client?  Let's
	...
	// accept connections from a Bvcse Chat v1.x client.
	...
	tmpOutput.println("/SERVERMESSAGE This server is incompatible "
			  + "with Bvcse Chat 1.x.  Please upgrade your "
    ...
    protected BvcseClientSocket findClientSocket(int userid)
    {
	// This will return the client socket object for the user with the
	// requested ID
	
	BvcseClientSocket tempClient = null;
	BvcseClientSocket returnClient = null;

	for (int count = 0; count < server.currentConnections; count ++)
	    {
		tempClient = (BvcseClientSocket)
	...
	for (int count = 0; count < chatRoom.canvasHistory.size(); count ++)
	    {
		BvcseCanvasCommand command;
		try {
		    command = (BvcseCanvasCommand)
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.CONNECT);
		ostream.writeUTF(userName);
	    }
    }
    
    protected void sendUserInfo(BvcseUser newuser)
	throws IOException
    {
	...
	if (server.requirePasswords)
		...
		try {
			...
			.checkPassword(BvcseServer.userPasswordFileName,
	...
	sendServerText("Welcome to Bvcse Chat version " + Bvcse.VERSION +
		       ".\n\n");
	...
	for (int count = 0; count < server.messages.size(); count ++)
	    if (((BvcseMessage) server.messages.elementAt(count))
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.SERVERMESS);
 	...
 	throws IOException
   {
	...
	synchronized (ostream)
		...
		for (int count1 = 0; count1 < server.chatRooms.size();
		     count1 ++)
		    {
			BvcseChatRoom room = (BvcseChatRoom)
 	...
 	throws IOException
   {
	...
	String roomName = "";
	BvcseChatRoom inviteRoom;
	int inviteUserId = 0;
	BvcseClientSocket inviteUser = null;
    ...
    protected void sendEnterRoom(int userId, BvcseChatRoom room)
	throws IOException
    {
	...
	boolean priv = false;
	String password = "";
	BvcseChatRoom newRoom;
	BvcseClientSocket tmpClient = null;
	...
	if (newRoom == null)
		...
		newRoom = new BvcseChatRoom(newRoomName, user.name, priv,
					      password);
		try {
	...
	String roomName = "";
	BvcseChatRoom bootRoom;
	int bootUserId = 0;
	BvcseClientSocket bootUser = null;
	...
	String roomName = "";
	BvcseChatRoom banRoom;
	int banUserId = 0;
	BvcseClientSocket banUser = null;
	...
	String roomName = "";
	BvcseChatRoom allowRoom;
	int allowUserId = 0;
	BvcseClientSocket allowUser = null;
	...
	chatRoom.canvasHistory.addElement(
		  new BvcseCanvasCommand(BvcseCommand.LINE, user.id,
	...
	chatRoom.canvasHistory.addElement(
		  new BvcseCanvasCommand(kind, user.id, colour, x, y, width,
	...
	chatRoom.canvasHistory.addElement(
		  new BvcseCanvasCommand(BvcseCommand.DRAWTEXT, user.id,
					   colour, x, y, (short) 0,
	...
	chatRoom.canvasHistory.addElement(
		  new BvcseCanvasCommand(BvcseCommand.DRAWPICTURE,
					   user.id, (short) 0, x, y,
	...
	for (int count = 0; count < server.currentConnections; count ++)
	    {
		BvcseClientSocket toUser = (BvcseClientSocket)
		    server.connections.elementAt(count);
	...
	server.messages.addElement(new BvcseMessage(whofor, user.name, 
							message));
    }
	...
	synchronized (ostream)
	    {
		ostream.writeShort(BvcseCommand.STOREDMESS);
		...
		for (int count = 0; count < server.messages.size(); count ++)
		    {
			message =
			    (BvcseMessage) server.messages.elementAt(count);
